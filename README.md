...
1. 音声合成ソフトウェア等で出力した音声データと、テキストファイルを用意します。
    ...
    - ファイル名は同一にして、拡張子だけを `.wav` と `.txt` にする必要があります（例: `000-AAAA-BBBB-CCCC.wav` と `000-AAAA-BBBB-CCCC.txt`）。
        - 上記ファイル名の`BBBB`に相当する部分は、字幕のレイヤー名になります。キャラクター名などを想定しています。
        - 字幕レイヤー名は前方一致で検索します。
            - `000-テスト-音街ウナ-000.wav`の場合、`音街ウナ`あるいは`音街ウナ字幕`のような名前のテキストレイヤーに字幕が生成されます。
    - テキストファイルの内容は、音声ファイルに対応する字幕のテキストです。
